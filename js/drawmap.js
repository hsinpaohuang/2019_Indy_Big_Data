...
function INmap(year)
{
	tip = d3.tip().attr('class', 'd3-tip').html(function(d) { return '<strong>County: </strong> <span class="details">' + d.properties.NAME + '<br /></span> <strong>Percent: </strong><span class="details">' + d.properties.Percent + '%</span>'; });
	svg = d3.select("#map-div")
			.append("svg")
			.attr("class", "map y"+year)
			.attr("width", width)
			.attr("height", height);
	d3.select("#map-div").select("g:nth-child(0)").remove();
	svg.call(tip);
	tip.direction('e');

	percent_data = [];
	d3.queue()
	.defer(d3.json,"js/indycounty.json")
	.defer(d3.csv, "data/percent/p_"+year+".csv", function(d)
		{
			percent_data.push(d);
		})//d3.csv
		.await(ready);
